...
async fn get_roads_in_bbox(
    ...
    let lon1 = query
        .get("lon1")
        .and_then(|lon1_str| lon1_str.parse::<f64>().ok())
        .unwrap_or_default();
    let lat1 = query
        .get("lat1")
        .and_then(|lat1_str| lat1_str.parse::<f64>().ok())
        .unwrap_or_default();
    let lon2 = query
        .get("lon2")
        .and_then(|lon2_str| lon2_str.parse::<f64>().ok())
        .unwrap_or_default();
    let lat2 = query
        .get("lat2")
        .and_then(|lat2_str| lat2_str.parse::<f64>().ok())
        .unwrap_or_default();
