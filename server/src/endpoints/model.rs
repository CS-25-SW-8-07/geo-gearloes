...
use rusty_roads::Model;
use std::{collections::HashMap, sync::mpsc::channel};

use actix_web::{
    dev::{ServiceFactory, ServiceRequest},
    get, web, App, Error, HttpRequest, HttpResponse, Responder,
};
use sqlx::PgPool;
...
pub fn services<T: ServiceFactory<ServiceRequest, Config = (), Error = Error, InitError = ()>>(
    ...
    app.service(get_model).service(update_model)
}

pub struct Clients {
    devices: Vec<(Sender<TtoCMessages>, Receiver<CtoTMessages>)>,
}

pub struct ModelTrainerTx(Sender<CtoTMessages>);

pub enum CtoTMessages {
    AddDevice(Sender<TtoCMessages>, Receiver<CtoTMessages>),
    TrainingDone,
}

#[derive(Debug, Eq, PartialEq)]
pub enum TtoCMessages {
    BeginTraining,
    SendModel(rusty_roads::Model),
}
...
impl Clients {
    ...
    pub fn add_client(&mut self, sender: Sender<TtoCMessages>, receiver: Receiver<CtoTMessages>) {
        self.devices.push((sender, receiver));
    }

    pub fn new_round(mut self) -> Clients {
        let clients = Clients {
...
#[get("/get_model")]
async fn get_model(pool: web::Data<PgPool>) -> impl Responder {
    let conn = match pool.acquire().await {
        Ok(c) => c,
        Err(_) => return HttpResponse::InternalServerError().body(""),
    };

    let model = match model::fetch_model(conn).await {
        Ok(v) => v,
        Err(_) => return HttpResponse::InternalServerError().body(""),
    };
...
async fn update_model(
    req: HttpRequest,
    body: web::Payload,
    trainer: web::Data<ModelTrainerTx>,
) -> actix_web::Result<impl Responder> {
    let (response, session, msg_stream) = actix_ws::handle(&req, body)?;

    let (sender, receiver) = channel::<TtoCMessages>();

    let (sender2, receiver2) = channel::<CtoTMessages>();

    // Add client to training round.
    trainer
        .into_inner()
        .0
        .send(CtoTMessages::AddDevice(sender, receiver2))
        .map_err(|x| actix_web::error::ErrorInternalServerError(x.to_string()))?;

    actix_web::rt::spawn(async move {
        if receiver
            .recv()
            .is_ok_and(|x| x == TtoCMessages::BeginTraining)
        {
...
fn training_officer((self_sender, receiver): (Sender<CtoTMessages>, Receiver<CtoTMessages>)) {
    let mut receiver_stream = receiver.iter();

    let mut training_ongoing: bool = false;

    let mut clients = Clients::new();

    while let Some(msg) = receiver_stream.next() {
        match msg {
            CtoTMessages::AddDevice(s, r) => {
                clients.add_client(s, r);

                if !training_ongoing && clients.len() > 10 {
                    training_ongoing = true;
                    let clients_in_round = clients;
                    clients = Clients::new();
                    let training_sender = self_sender.clone();
                    std::thread::spawn(move || {
                        start_training(clients_in_round, training_sender);
                    });
                }
            }
            CtoTMessages::TrainingDone => {
                if clients.len() > 10 {
                    let clients_in_round = clients;
                    clients = Clients::new();
                    let training_sender = self_sender.clone();
...
fn start_training(clients: Clients, sender: Sender<CtoTMessages>) {
    clients
        .devices
        .iter()
        .for_each(|(s, r)| s.send(TtoCMessages::BeginTraining));

    let models: Vec<rusty_roads::Model> = clients
        .devices
        .iter()
        .filter_map(|(s, r)| r.recv().ok())
        .collect();
}
